(function() {
	...
	/** 
	 * Parses input source (of Markdown syntax) and converts into Object 
	 * with containing HTML equivalent content it's matter data.
	 *
	 * @param src String containing the Markdown to parse and convert
	...
	function convert (src, opts, callback) {
		...
		// just content, with no front matter
		var content = src.replace(frontMatterRE, '');
		// parse content to HTML (done by external lib marked.js)
		_parser(content, opts, function (err, html) {
			if(err)
				callback(err);
			else {
				_parseFrontMatter(src, function(err, matter) {
					callback(null, {
						content: html,
						title: matter.title,
						tags: matter.tags,
	 ...
	 */
	var converter = function (note, opts, callback) {
		if(_isUndefined(callback) && _isFunction(opts)) {
			callback = opts;
			opts = {};	
		}

		if(_isUndefined(note) || _isUndefined(note.content)) {
			callback(new Error('Note is undefined!'));
			return;
		}

		convert(note.content, opts, function (err, converted) {
